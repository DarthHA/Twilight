...
namespace Twilight.Projectiles
{
    public class EyeLantern2 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            Projectile.Opacity = Projectile.ai[1] / 40f;
        }
        ...
        private void DrawLantern()
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                if (CircleR[i] > 0)
                {
                    ...
                    DrawData data = new DrawData(ModContent.Request<Texture2D>("Twilight/Images/Extra_193").Value, ScreenPos, new Rectangle?(new Rectangle(0, 0, (int)(R * 2), (int)(R * 2))), Color.Orange * ops * Projectile.Opacity, Projectile.rotation, new Vector2(R, R), new Vector2(1, 0.67f), SpriteEffects.None, 0);
        ...
        public static int FindLantern2()
        {
            foreach (Projectile proj in Main.projectile)
            {
                if (proj.active && proj.type == ModContent.ProjectileType<EyeLantern2>() && proj.ai[0] == 0)
                {
                    return proj.whoAmI;
                }
            }
        ...
        public static void CloseLantern2()
        {
            foreach (Projectile proj in Main.projectile)
            {
